<x-admin-panel::guest-layout>
    <x-admin-panel::authentication-card>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div>
                <x-admin-panel::label for="name" value="{{ __('Name') }}" />
                <x-admin-panel::input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
            </div>

            <div class="mt-4">
                <x-admin-panel::label for="email" value="{{ __('Email') }}" />
                <x-admin-panel::input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email')" required autocomplete="username" />
            </div>
            ...
            @if (Laravel\Jetstream\Jetstream::hasTermsAndPrivacyPolicyFeature())
                <div class="mt-4">
                    <x-admin-panel::label for="terms">
                        <div class="flex items-center">
                            <x-admin-panel::checkbox name="terms" id="terms" required />
